...
const firebase = require('./firebase_cert.json');
...
function missions_choose(gioco_uno, gioco_due, gioco_tre, gioco_quattro, ms_obj)
{
	var giocouno = "";
	var giocodue = "";
	var giocotre = "";
	...
	switch(giocouno.substr(giocouno.length - 5))
	{
		...
		default:
		client.users.cache.get(koray_id).send("null_shadows_1");
	}
	...
	switch(giocodue.substr(giocodue.length - 5))
	{
		...
		default:
		client.users.cache.get(koray_id).send("null_shadows_2");
	}
	...
	switch(giocotre.substr(giocotre.length - 5))
	{
		...
		default:
		client.users.cache.get(koray_id).send("null_shadows_3");
	}
	...
	switch(giocoquattro.substr(giocoquattro.length - 5))
	{
		...
		default:
		client.users.cache.get(koray_id).send("null_shadows_4");
	}
...
client.on('ready', async () => {
	...
	// prefix = (await db.collection('others').doc('prefix').get()).data().p;
	...
	scheduler.scheduleJob('0 0 19 * * *', async ()=>{
		var d = new Date();
		...
		var csv = await request(url_today_covid);
		var csvy = await request(url_today_covidy);
		//console.log(csv);
		//console.log(csvy);
		csv = csv.split('\n')[1].split(',');
		csvy = csvy.split('\n')[1].split(',');
		...
		var msg_covid = `casi totali: \`${cases}\` (+\`${newcases}\`)\n`+
				`positivià: \`${perc}%\`\n`+
				`tamponi: \`${testst}\` (+\`${newtests}\`)\n`+
				`morti: \`${deaths}\` (+\`${newdeaths}\`)\n`+
				`terapia intensiva: \`${terint}\` (+\`${newterint}\`)`;
	...
	scheduler.scheduleJob('0 0 9 27 3 *', ()=>{
		client.channels.cache.get(id_general_channel).send(bdays_msgs.timw);
		console.log('sent best wishes timw on '+new Date());
	});
	//koray
	scheduler.scheduleJob('0 30 8 17 5 *', ()=>{
		client.channels.cache.get(id_general_channel).send(bdays_msgs.koray);
		console.log('sent best wishes koray on '+new Date());
	});
	//triccotricco
	scheduler.scheduleJob('0 30 8 6 5 *', ()=>{
		client.channels.cache.get(id_general_channel).send(bdays_msgs.triccotricco);
		console.log('sent best wishes triccotricco on '+new Date());
	});
	//lux
	scheduler.scheduleJob('0 30 8 16 11 *', ()=>{
		client.channels.cache.get(id_general_channel).send(bdays_msgs.lux);
		console.log('sent best wishes lux on '+new Date());
	});
	//xevery
	scheduler.scheduleJob('0 30 8 15 1 *', ()=>{
		client.channels.cache.get(id_general_channel).send(bdays_msgs.xevery);
		console.log('sent best wishes xevery on '+new Date());
	});
	//creepraptor (creep)
	scheduler.scheduleJob('0 30 8 26 10 *', ()=>{
		client.users.cache.get(creep_id).send(bdays_msgs.creep);
		console.log('sent best wishes creepraptor (in dm) on '+new Date());
	});
	//cb (CB7356)
	scheduler.scheduleJob('0 30 8 18 5 *', ()=>{
		client.users.cache.get(cb_id).send(bdays_msgs.cb);
...
client.on('message', async message => {try{
	if(message.content.startsWith(p+'edit-mission ')){
		if(check_perms(message,'a')){
			...
			channel_missions_id = (await db.collection('missions_files').doc('missions').get()).data().channel_id;
	...
	if(message.content.startsWith(p+'scheda') || message.content.startsWith(p+'card'))
	{
		...
		if(message.content == '/scheda list' || message.content == '/card list'){
			...
			{
				lista += `- **${client.users.cache.get(key).tag}**\n`;
			}
	...
	if(message.mentions.users.find(u=>u.id == bot_id) || message.channel.type == 'dm')
	{
		...
		if((/grz/i).test(m) && m.length >= min_len){
			...
			r=rn(a)+((Math.floor(Math.random()*15)==Math.floor(Math.random()*10))?" (Anche se si scrive 'grazie')":'');
			message.reply(r);
			console.log("'"+message.content+"' from "+message.author.id+"; ans: '"+r+"'");
		}
			...
			message.reply('np');
			console.log("'"+message.content+"' from "+message.author.id+"; ans: 'np'");
		}
		//thanking is no more
		...
		//insult/compliment
			...
			/* const list_ins_comp = {"insults": [/figlio d(e|i) put/, "scem", "bastard", "coglion", "testa di cazzo", "lurid", "lurido schifos", "inutil", "porc", "nan", /str(o|u)nz/], "compliments": ["brav", "intelligent", "bell", "util"]};
			const risposte = {"insults": ["Altrettano", "Tu sei un coglione", "Tu sei un coglione", "Tu sei uno scemo", "Tu sei un bastardo", "Tu sei un figlio di... brava donna", "Tu sei uno scemo", "Vaffanculo", "A bucchin a mammt"], "compliments": ["Altrettano", "Tu sei bravo", "Tu sei intelligente", "Tu sei bello", "Tu sei utile", ":) Grazie", ":) Grazie Mille", "Molte Grazie :)", "Thank you", "ty"]}; */
			const list_ins_comp = (await db.collection('others').doc('inputs').get()).data();
			const risposte = (await db.collection('others').doc('answers').get()).data();
			insult = false;
			...
			for(i in list_ins_comp.insults){
				for(j in m){
					if(m[j].match(new RegExp(list_ins_comp.insults[i]),'i')){
						insult = true;
						m[j].match(new RegExp(list_ins_comp.insults[i]),'i')
					}
				}
			}
			for(i in list_ins_comp.compliments){
				for(j in m){
					if(m[j].match(new RegExp(list_ins_comp.compliments[i],'i'))){
						complim = true;
						m[j].match(new RegExp(list_ins_comp.compliments[i],'i'))
					}
				}
			}
			if(insult){risp = risposte.insults[Math.floor(Math.random() * risposte.insults.length)]};
			if(complim){risp = risposte.compliments[Math.floor(Math.random() * risposte.compliments.length)]};
			// console.log(risp);
			if(risp != 0)message.channel.send(risp);
			console.log("'"+message.content+"' from "+message.author.id+"; ans: '"+risp+"'");
		}
	}
	...
	if(message.content.startsWith(p+'clear-byid-'))
	{
		if(check_perms(message,'a'))
		{
			...
			for(i=0;i<cont.length;i++){
				...
				message.channel.messages.fetch(id).then(
				mes_clear => {
					console.log(mes_clear.id);
					mes_clear.delete().then(msg => {var d = Date(); console.log(`Deleted clear-byid message from ${msg.author.username} at ${d} by ${cm.author.tag}`)}).catch(console.error);
					// cm.delete().then(msg => {var d = Date(); console.log(`Deleted message from ${msg.author.username} at ${d}`)}).catch(console.error);
				},
				reas => {
					cm.author.send("Didn't find the message");
					// cm.delete().then(msg => {var d = Date(); console.log(`Deleted message from ${msg.author.username} at ${d}`)}).catch(console.error);
				}).catch(console.error);
			}
		}
	...
	if(message.content == p+'hjelp')
	{
		if(check_perms(message,'a'))
		{
			message.author.send(">>> **Commands**\n\n"
								...
								+"`/edit-mission [game] [new mission]` OR\n"
								+"`/edit-mission [game] s/ [pattern to subtitute] [subtitute with]`\n"
	...
	if(message.content == 'poll_missions_send' && (message.author.id === bot_id || message.author.id === koray_id)){
		...
		client.users.cache.get(koray_id).send(`sent poll at ${Date()}`);
	...
	if(message.content === 'missions_activate_now' && (message.author.id === bot_id || message.author.id === koray_id))
	{
		missions_file = order_obj((await db.collection('missions_files').doc('missions_file').get()).data());
		message.delete().then(msg => {var d = Date(); console.log(`Deleted message from ${msg.author.username} at ${d}`)}).catch(console.error);
		...
		setTimeout(()=>{
			...
			client.users.cache.get(koray_id).send("check <#437961671018020864>");
		}, 10000)
	}
